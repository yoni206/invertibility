...
(or (bvsge (udivtotal (bvnot (_ bv0 65)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 65))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 65))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
