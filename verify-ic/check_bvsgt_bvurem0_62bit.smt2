...
(and (and (=> (bvsgt s (_ bv0 62)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 62)) (distinct t max))) (or (distinct t (_ bv0 62)) (distinct s (_ bv1 62))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 62))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 62))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
