...
(define-fun SC () Bool
(and (bvuge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 46)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 46))) (bvugt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 46))) (bvugt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
