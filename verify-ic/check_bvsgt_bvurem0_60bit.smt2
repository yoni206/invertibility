...
(and (and (=> (bvsgt s (_ bv0 60)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 60)) (distinct t max))) (or (distinct t (_ bv0 60)) (distinct s (_ bv1 60))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 60))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 60))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
