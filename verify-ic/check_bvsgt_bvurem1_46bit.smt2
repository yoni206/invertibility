...
(and (=> (bvsge s (_ bv0 46)) (bvsgt s t)) (=> (bvslt s (_ bv0 46)) (bvsgt (bvlshr (bvsub s (_ bv1 46)) (_ bv1 46)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 46))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 46))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
