...
(and (=> (bvsge s (_ bv0 38)) (bvsge s t)) (=> (bvslt s (_ bv0 38)) (bvsge (bvlshr s (_ bv1 38)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 38))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 38))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
