...
(define-fun SC () Bool
(and (bvsge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 34)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 34))) (bvsgt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 34))) (bvsgt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
