...
(and (=> (bvsge s (_ bv0 43)) (bvsgt s t)) (=> (bvslt s (_ bv0 43)) (bvsgt (bvlshr (bvsub s (_ bv1 43)) (_ bv1 43)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 43))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 43))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
