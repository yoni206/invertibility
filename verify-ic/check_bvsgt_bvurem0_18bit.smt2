...
(and (and (=> (bvsgt s (_ bv0 18)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 18)) (distinct t max))) (or (distinct t (_ bv0 18)) (distinct s (_ bv1 18))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 18))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 18))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
