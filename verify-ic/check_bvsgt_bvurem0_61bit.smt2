...
(and (and (=> (bvsgt s (_ bv0 61)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 61)) (distinct t max))) (or (distinct t (_ bv0 61)) (distinct s (_ bv1 61))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 61))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 61))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
