...
(and (=> (bvsge s (_ bv0 42)) (bvsgt s t)) (=> (bvslt s (_ bv0 42)) (bvsgt (bvlshr (bvsub s (_ bv1 42)) (_ bv1 42)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 42))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 42))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
