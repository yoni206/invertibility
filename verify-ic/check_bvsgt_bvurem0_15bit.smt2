...
(and (and (=> (bvsgt s (_ bv0 15)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 15)) (distinct t max))) (or (distinct t (_ bv0 15)) (distinct s (_ bv1 15))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 15))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 15))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
