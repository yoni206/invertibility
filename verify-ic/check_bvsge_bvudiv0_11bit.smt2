...
(or (bvsge (udivtotal (bvnot (_ bv0 11)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 11))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 11))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
