...
(and (and (=> (bvsgt s (_ bv0 28)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 28)) (distinct t max))) (or (distinct t (_ bv0 28)) (distinct s (_ bv1 28))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 28))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 28))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
