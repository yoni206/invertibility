...
(or (bvsgt (udivtotal (bvnot (_ bv0 28)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 28))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 28))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
