...
(and (=> (bvslt s (_ bv0 43)) (bvsgt (bvlshr s (_ bv1 43)) t)) (=> (bvsge s (_ bv0 43)) (bvsgt s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 43))) (bvsgt (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec 43))) (bvsgt (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
