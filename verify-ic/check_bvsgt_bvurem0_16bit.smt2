...
(and (and (=> (bvsgt s (_ bv0 16)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 16)) (distinct t max))) (or (distinct t (_ bv0 16)) (distinct s (_ bv1 16))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 16))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 16))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
