...
(define-fun SC () Bool
(and (bvsge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 53)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 53))) (bvsgt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 53))) (bvsgt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
