...
(and (and (=> (bvsgt s (_ bv0 39)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 39)) (distinct t max))) (or (distinct t (_ bv0 39)) (distinct s (_ bv1 39))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 39))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 39))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
