...
(and (=> (bvsge s (_ bv0 24)) (bvsgt s t)) (=> (bvslt s (_ bv0 24)) (bvsgt (bvlshr (bvsub s (_ bv1 24)) (_ bv1 24)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 24))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 24))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
