...
(define-fun SC () Bool
(and (bvuge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 28)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 28))) (bvugt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 28))) (bvugt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
