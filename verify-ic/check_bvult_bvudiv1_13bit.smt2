...
(and (bvult (_ bv0 13) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 13) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 13))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 13))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
