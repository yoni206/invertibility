...
(and (=> (bvsge s (_ bv0 56)) (bvsge s t)) (=> (bvslt s (_ bv0 56)) (bvsge (bvlshr s (_ bv1 56)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 56))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 56))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
