...
(and (and (=> (bvsgt s (_ bv0 30)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 30)) (distinct t max))) (or (distinct t (_ bv0 30)) (distinct s (_ bv1 30))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 30))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 30))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
