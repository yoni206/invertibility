...
(and (=> (bvsge s (_ bv0 1)) (bvsgt s t)) (=> (bvslt s (_ bv0 1)) (bvsgt (bvlshr (bvsub s (_ bv1 1)) (_ bv1 1)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 1))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 1))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
