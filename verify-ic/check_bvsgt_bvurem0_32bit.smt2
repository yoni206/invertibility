...
(and (and (=> (bvsgt s (_ bv0 32)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 32)) (distinct t max))) (or (distinct t (_ bv0 32)) (distinct s (_ bv1 32))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 32))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 32))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
