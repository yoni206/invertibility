...
(and (=> (bvslt s (_ bv0 48)) (bvsge (bvlshr s (_ bv1 48)) t)) (=> (bvsge s (_ bv0 48)) (bvsge s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 48))) (bvsge (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec 48))) (bvsge (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
