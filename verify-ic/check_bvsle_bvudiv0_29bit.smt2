...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (_ bv0 29)) (bvslt (udivtotal min s) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 29))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 29))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
