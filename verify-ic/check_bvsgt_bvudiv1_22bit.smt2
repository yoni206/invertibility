...
(and (=> (bvsge s (_ bv0 22)) (bvsgt s t)) (=> (bvslt s (_ bv0 22)) (bvsgt (bvlshr s (_ bv1 22)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 22))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 22))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
