...
(and (=> (bvsge s (_ bv0 53)) (bvsgt s t)) (=> (bvslt s (_ bv0 53)) (bvsgt (bvlshr s (_ bv1 53)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 53))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 53))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
