...
(and (=> (bvsge s (_ bv0 41)) (bvsge s t)) (=> (bvslt s (_ bv0 41)) (bvsge (bvlshr s (_ bv1 41)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 41))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 41))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
