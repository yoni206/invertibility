...
(and (=> (bvsge s (_ bv0 63)) (bvsge s t)) (=> (bvslt s (_ bv0 63)) (bvsge (bvlshr s (_ bv1 63)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 63))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 63))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
