...
(and (and (=> (bvsgt s (_ bv0 11)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 11)) (distinct t max))) (or (distinct t (_ bv0 11)) (distinct s (_ bv1 11))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 11))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 11))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
