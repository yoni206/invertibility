...
(and (bvult (_ bv0 59) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 59) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 59))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 59))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
