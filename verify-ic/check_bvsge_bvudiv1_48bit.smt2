...
(and (=> (bvsge s (_ bv0 48)) (bvsge s t)) (=> (bvslt s (_ bv0 48)) (bvsge (bvlshr s (_ bv1 48)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 48))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 48))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
