...
(and (=> (bvsge s (_ bv0 46)) (bvsge s t)) (=> (bvslt s (_ bv0 46)) (bvsge (bvlshr s (_ bv1 46)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 46))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 46))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
