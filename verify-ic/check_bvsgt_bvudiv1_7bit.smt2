...
(and (=> (bvsge s (_ bv0 7)) (bvsgt s t)) (=> (bvslt s (_ bv0 7)) (bvsgt (bvlshr s (_ bv1 7)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 7))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 7))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
