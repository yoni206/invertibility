...
(and (=> (bvsge s (_ bv0 28)) (bvsgt s t)) (=> (bvslt s (_ bv0 28)) (bvsgt (bvlshr (bvsub s (_ bv1 28)) (_ bv1 28)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 28))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 28))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
