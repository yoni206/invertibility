...
(define-fun SC () Bool
(and (bvuge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 55)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 55))) (bvugt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 55))) (bvugt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
