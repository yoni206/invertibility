...
(and (=> (bvsge s (_ bv0 51)) (bvsgt s t)) (=> (bvslt s (_ bv0 51)) (bvsgt (bvlshr (bvsub s (_ bv1 51)) (_ bv1 51)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 51))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 51))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
