...
(and (=> (bvslt s (_ bv0 20)) (bvsgt (bvlshr s (_ bv1 20)) t)) (=> (bvsge s (_ bv0 20)) (bvsgt s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 20))) (bvsgt (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec 20))) (bvsgt (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
