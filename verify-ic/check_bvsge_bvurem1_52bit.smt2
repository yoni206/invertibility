...
(and (=> (bvsge s (_ bv0 52)) (bvsge s t)) (=> (and (bvslt s (_ bv0 52)) (bvsge t (_ bv0 52))) (bvugt (bvsub s t) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 52))) (bvsge (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 52))) (bvsge (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
