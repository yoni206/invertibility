...
(and (and (=> (bvsgt s (_ bv0 51)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 51)) (distinct t max))) (or (distinct t (_ bv0 51)) (distinct s (_ bv1 51))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 51))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 51))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
