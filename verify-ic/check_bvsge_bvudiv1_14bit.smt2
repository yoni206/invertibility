...
(and (=> (bvsge s (_ bv0 14)) (bvsge s t)) (=> (bvslt s (_ bv0 14)) (bvsge (bvlshr s (_ bv1 14)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 14))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 14))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
