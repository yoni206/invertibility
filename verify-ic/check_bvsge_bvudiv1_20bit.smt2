...
(and (=> (bvsge s (_ bv0 20)) (bvsge s t)) (=> (bvslt s (_ bv0 20)) (bvsge (bvlshr s (_ bv1 20)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 20))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 20))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
