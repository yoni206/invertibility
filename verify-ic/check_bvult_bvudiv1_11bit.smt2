...
(and (bvult (_ bv0 11) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 11) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 11))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 11))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
