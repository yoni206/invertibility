...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (_ bv0 46)) (bvslt (udivtotal min s) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 46))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 46))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
