...
(or (bvsge (udivtotal (bvnot (_ bv0 2)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 2))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 2))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
