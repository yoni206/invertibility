...
(and (=> (bvsge s (_ bv0 18)) (bvsge s t)) (=> (bvslt s (_ bv0 18)) (bvsge (bvlshr s (_ bv1 18)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 18))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 18))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
