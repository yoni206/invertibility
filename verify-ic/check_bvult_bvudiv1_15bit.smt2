...
(and (bvult (_ bv0 15) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 15) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 15))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 15))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
