...
(or (bvsge (udivtotal (bvnot (_ bv0 64)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 64))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 64))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
