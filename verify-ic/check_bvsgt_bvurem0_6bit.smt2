...
(and (and (=> (bvsgt s (_ bv0 6)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 6)) (distinct t max))) (or (distinct t (_ bv0 6)) (distinct s (_ bv1 6))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 6))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 6))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
