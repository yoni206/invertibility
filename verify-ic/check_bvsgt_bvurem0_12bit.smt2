...
(and (and (=> (bvsgt s (_ bv0 12)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 12)) (distinct t max))) (or (distinct t (_ bv0 12)) (distinct s (_ bv1 12))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 12))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 12))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
