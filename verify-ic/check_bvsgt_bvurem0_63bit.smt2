...
(and (and (=> (bvsgt s (_ bv0 63)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 63)) (distinct t max))) (or (distinct t (_ bv0 63)) (distinct s (_ bv1 63))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 63))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 63))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
