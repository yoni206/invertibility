...
(and (and (=> (bvsgt s (_ bv0 8)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 8)) (distinct t max))) (or (distinct t (_ bv0 8)) (distinct s (_ bv1 8))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 8))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 8))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
