...
(and (=> (bvsge s (_ bv0 40)) (bvsgt s t)) (=> (bvslt s (_ bv0 40)) (bvsgt (bvlshr s (_ bv1 40)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 40))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 40))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
