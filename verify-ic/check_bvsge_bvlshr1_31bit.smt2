...
(and (=> (bvslt s (_ bv0 31)) (bvsge (bvlshr s (_ bv1 31)) t)) (=> (bvsge s (_ bv0 31)) (bvsge s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 31))) (bvsge (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec 31))) (bvsge (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
