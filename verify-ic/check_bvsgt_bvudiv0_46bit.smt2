...
(or (bvsgt (udivtotal (bvnot (_ bv0 46)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 46))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 46))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
