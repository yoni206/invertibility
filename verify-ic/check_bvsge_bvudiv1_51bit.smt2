...
(and (=> (bvsge s (_ bv0 51)) (bvsge s t)) (=> (bvslt s (_ bv0 51)) (bvsge (bvlshr s (_ bv1 51)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 51))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 51))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
