...
(and (=> (bvsge s (_ bv0 62)) (bvsge s t)) (=> (bvslt s (_ bv0 62)) (bvsge (bvlshr s (_ bv1 62)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 62))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 62))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
