...
(and (and (=> (bvsgt s (_ bv0 24)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 24)) (distinct t max))) (or (distinct t (_ bv0 24)) (distinct s (_ bv1 24))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 24))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 24))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
