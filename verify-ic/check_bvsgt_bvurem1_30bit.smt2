...
(and (=> (bvsge s (_ bv0 30)) (bvsgt s t)) (=> (bvslt s (_ bv0 30)) (bvsgt (bvlshr (bvsub s (_ bv1 30)) (_ bv1 30)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 30))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 30))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
