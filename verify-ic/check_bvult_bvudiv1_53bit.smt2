...
(and (bvult (_ bv0 53) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 53) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 53))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 53))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
