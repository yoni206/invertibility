...
(and (and (=> (bvsgt s (_ bv0 42)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 42)) (distinct t max))) (or (distinct t (_ bv0 42)) (distinct s (_ bv1 42))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 42))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 42))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
