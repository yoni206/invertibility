...
(and (=> (bvsge s (_ bv0 12)) (bvsge s t)) (=> (bvslt s (_ bv0 12)) (bvsge (bvlshr s (_ bv1 12)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 12))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 12))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
