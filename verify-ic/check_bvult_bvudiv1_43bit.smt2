...
(and (bvult (_ bv0 43) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 43) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 43))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 43))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
