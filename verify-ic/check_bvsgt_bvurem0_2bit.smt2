...
(and (and (=> (bvsgt s (_ bv0 2)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 2)) (distinct t max))) (or (distinct t (_ bv0 2)) (distinct s (_ bv1 2))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 2))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 2))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
