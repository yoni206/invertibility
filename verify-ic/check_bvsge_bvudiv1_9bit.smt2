...
(and (=> (bvsge s (_ bv0 9)) (bvsge s t)) (=> (bvslt s (_ bv0 9)) (bvsge (bvlshr s (_ bv1 9)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 9))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 9))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
