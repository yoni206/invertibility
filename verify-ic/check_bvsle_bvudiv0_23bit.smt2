...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (_ bv0 23)) (bvslt (udivtotal min s) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 23))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 23))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
