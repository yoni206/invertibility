...
(and (=> (bvsge s (_ bv0 21)) (bvsge s t)) (=> (bvslt s (_ bv0 21)) (bvsge (bvlshr s (_ bv1 21)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 21))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 21))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
