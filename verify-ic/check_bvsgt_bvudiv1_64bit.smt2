...
(and (=> (bvsge s (_ bv0 64)) (bvsgt s t)) (=> (bvslt s (_ bv0 64)) (bvsgt (bvlshr s (_ bv1 64)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 64))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 64))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
