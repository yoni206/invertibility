...
(and (=> (bvsge s (_ bv0 15)) (bvsge s t)) (=> (bvslt s (_ bv0 15)) (bvsge (bvlshr s (_ bv1 15)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 15))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 15))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
