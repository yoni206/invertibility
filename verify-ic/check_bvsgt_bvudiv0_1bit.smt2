...
(or (bvsgt (udivtotal (bvnot (_ bv0 1)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 1))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 1))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
