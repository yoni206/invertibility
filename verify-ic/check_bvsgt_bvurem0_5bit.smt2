...
(and (and (=> (bvsgt s (_ bv0 5)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 5)) (distinct t max))) (or (distinct t (_ bv0 5)) (distinct s (_ bv1 5))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 5))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 5))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
