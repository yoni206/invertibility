...
(and (and (=> (bvsgt s (_ bv0 41)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 41)) (distinct t max))) (or (distinct t (_ bv0 41)) (distinct s (_ bv1 41))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 41))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 41))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
