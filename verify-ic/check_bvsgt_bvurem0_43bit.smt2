...
(and (and (=> (bvsgt s (_ bv0 43)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 43)) (distinct t max))) (or (distinct t (_ bv0 43)) (distinct s (_ bv1 43))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 43))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 43))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
