...
(and (and (=> (bvsgt s (_ bv0 38)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 38)) (distinct t max))) (or (distinct t (_ bv0 38)) (distinct s (_ bv1 38))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 38))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 38))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
