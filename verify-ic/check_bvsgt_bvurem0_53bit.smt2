...
(and (and (=> (bvsgt s (_ bv0 53)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 53)) (distinct t max))) (or (distinct t (_ bv0 53)) (distinct s (_ bv1 53))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 53))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 53))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
