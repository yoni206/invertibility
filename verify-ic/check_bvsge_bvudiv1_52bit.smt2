...
(and (=> (bvsge s (_ bv0 52)) (bvsge s t)) (=> (bvslt s (_ bv0 52)) (bvsge (bvlshr s (_ bv1 52)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 52))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 52))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
