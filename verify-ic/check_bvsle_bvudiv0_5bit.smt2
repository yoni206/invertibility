...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (_ bv0 5)) (bvslt (udivtotal min s) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 5))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 5))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
