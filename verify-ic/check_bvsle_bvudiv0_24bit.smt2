...
(or (= (udivtotal (bvmul s t) s) t) (=> (bvsle t (_ bv0 24)) (bvslt (udivtotal min s) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 24))) (bvsle (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 24))) (bvsle (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
