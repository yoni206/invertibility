...
(and (=> (bvslt s (_ bv0 38)) (bvsgt (bvlshr s (_ bv1 38)) t)) (=> (bvsge s (_ bv0 38)) (bvsgt s t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 38))) (bvsgt (bvlshr s x) t)))
  (=> (exists ((x (_ BitVec 38))) (bvsgt (bvlshr s x) t)) (SC s t))
  )
 )
)
(check-sat)
