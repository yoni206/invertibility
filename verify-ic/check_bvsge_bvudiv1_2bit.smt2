...
(and (=> (bvsge s (_ bv0 2)) (bvsge s t)) (=> (bvslt s (_ bv0 2)) (bvsge (bvlshr s (_ bv1 2)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 2))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 2))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
