...
(define-fun SC () Bool
(and (bvsge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 18)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 18))) (bvsgt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 18))) (bvsgt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
