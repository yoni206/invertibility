...
(define-fun SC () Bool
(and (bvuge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 64)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 64))) (bvugt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 64))) (bvugt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
