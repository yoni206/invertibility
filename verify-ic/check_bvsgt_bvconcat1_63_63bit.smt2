...
(define-fun SC () Bool
(and (bvsge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 63)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 63))) (bvsgt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 63))) (bvsgt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
