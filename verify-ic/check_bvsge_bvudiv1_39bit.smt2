...
(and (=> (bvsge s (_ bv0 39)) (bvsge s t)) (=> (bvslt s (_ bv0 39)) (bvsge (bvlshr s (_ bv1 39)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 39))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 39))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
