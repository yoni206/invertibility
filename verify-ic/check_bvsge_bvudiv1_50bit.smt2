...
(and (=> (bvsge s (_ bv0 50)) (bvsge s t)) (=> (bvslt s (_ bv0 50)) (bvsge (bvlshr s (_ bv1 50)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 50))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 50))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
