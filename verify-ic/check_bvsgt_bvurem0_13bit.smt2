...
(and (and (=> (bvsgt s (_ bv0 13)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 13)) (distinct t max))) (or (distinct t (_ bv0 13)) (distinct s (_ bv1 13))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 13))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 13))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
