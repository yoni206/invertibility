...
(and (and (=> (bvsgt s (_ bv0 7)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 7)) (distinct t max))) (or (distinct t (_ bv0 7)) (distinct s (_ bv1 7))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 7))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 7))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
