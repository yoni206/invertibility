...
(and (and (=> (bvsgt s (_ bv0 10)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 10)) (distinct t max))) (or (distinct t (_ bv0 10)) (distinct s (_ bv1 10))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 10))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 10))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
