...
(and (=> (bvsge s (_ bv0 22)) (bvsge s t)) (=> (bvslt s (_ bv0 22)) (bvsge (bvlshr s (_ bv1 22)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 22))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 22))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
