...
(and (=> (bvsge s (_ bv0 6)) (bvsge s t)) (=> (bvslt s (_ bv0 6)) (bvsge (bvlshr s (_ bv1 6)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 6))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 6))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
