...
(and (=> (bvsge s (_ bv0 25)) (bvsge s t)) (=> (bvslt s (_ bv0 25)) (bvsge (bvlshr s (_ bv1 25)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 25))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 25))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
