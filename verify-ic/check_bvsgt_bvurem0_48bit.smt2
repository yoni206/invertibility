...
(and (and (=> (bvsgt s (_ bv0 48)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 48)) (distinct t max))) (or (distinct t (_ bv0 48)) (distinct s (_ bv1 48))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 48))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 48))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
