...
(and (and (=> (bvsgt s (_ bv0 40)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 40)) (distinct t max))) (or (distinct t (_ bv0 40)) (distinct s (_ bv1 40))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 40))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 40))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
