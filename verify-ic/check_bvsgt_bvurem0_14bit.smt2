...
(and (and (=> (bvsgt s (_ bv0 14)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 14)) (distinct t max))) (or (distinct t (_ bv0 14)) (distinct s (_ bv1 14))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 14))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 14))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
