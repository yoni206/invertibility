...
(and (=> (bvsge s (_ bv0 52)) (bvsgt s t)) (=> (bvslt s (_ bv0 52)) (bvsgt (bvlshr (bvsub s (_ bv1 52)) (_ bv1 52)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 52))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 52))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
