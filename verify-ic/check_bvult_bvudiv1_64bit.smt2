...
(and (bvult (_ bv0 64) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 64) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 64))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 64))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
