...
(and (=> (bvsge s (_ bv0 34)) (bvsge s t)) (=> (bvslt s (_ bv0 34)) (bvsge (bvlshr s (_ bv1 34)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 34))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 34))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
