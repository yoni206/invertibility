...
(or (bvsge (udivtotal (bvnot (_ bv0 48)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 48))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 48))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
