...
(and (=> (bvsge s (_ bv0 13)) (bvsge s t)) (=> (bvslt s (_ bv0 13)) (bvsge (bvlshr s (_ bv1 13)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 13))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 13))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
