...
(and (=> (bvsge s (_ bv0 37)) (bvsge s t)) (=> (bvslt s (_ bv0 37)) (bvsge (bvlshr s (_ bv1 37)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 37))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 37))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
