...
(and (and (=> (bvsgt s (_ bv0 59)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 59)) (distinct t max))) (or (distinct t (_ bv0 59)) (distinct s (_ bv1 59))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 59))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 59))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
