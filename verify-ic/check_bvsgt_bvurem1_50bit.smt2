...
(and (=> (bvsge s (_ bv0 50)) (bvsgt s t)) (=> (bvslt s (_ bv0 50)) (bvsgt (bvlshr (bvsub s (_ bv1 50)) (_ bv1 50)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 50))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 50))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
