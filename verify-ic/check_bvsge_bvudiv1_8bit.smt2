...
(and (=> (bvsge s (_ bv0 8)) (bvsge s t)) (=> (bvslt s (_ bv0 8)) (bvsge (bvlshr s (_ bv1 8)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 8))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 8))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
