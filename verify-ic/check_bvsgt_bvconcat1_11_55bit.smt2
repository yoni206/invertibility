...
(define-fun SC () Bool
(and (bvsge s ts) (=> (= s ts) (distinct tx (bvnot (_ bv0 11)))))
)

(assert
 (not
  (and
   (=> SC (exists ((x (_ BitVec 11))) (bvsgt (concat s x) (concat ts tx))))
   (=> (exists ((x (_ BitVec 11))) (bvsgt (concat s x) (concat ts tx))) SC)
  )
 )
)
(check-sat)
