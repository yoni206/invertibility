...
(and (bvult (_ bv0 20) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 20) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 20))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 20))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
