...
(and (bvult (_ bv0 61) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 61) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 61))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 61))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
