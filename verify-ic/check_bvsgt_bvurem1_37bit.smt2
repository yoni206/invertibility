...
(and (=> (bvsge s (_ bv0 37)) (bvsgt s t)) (=> (bvslt s (_ bv0 37)) (bvsgt (bvlshr (bvsub s (_ bv1 37)) (_ bv1 37)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 37))) (bvsgt (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 37))) (bvsgt (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
