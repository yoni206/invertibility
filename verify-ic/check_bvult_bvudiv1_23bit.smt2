...
(and (bvult (_ bv0 23) (bvnot (bvand (bvneg t) s))) (bvult (_ bv0 23) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 23))) (bvult (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 23))) (bvult (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
