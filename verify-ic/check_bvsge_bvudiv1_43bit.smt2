...
(and (=> (bvsge s (_ bv0 43)) (bvsge s t)) (=> (bvslt s (_ bv0 43)) (bvsge (bvlshr s (_ bv1 43)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 43))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 43))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
