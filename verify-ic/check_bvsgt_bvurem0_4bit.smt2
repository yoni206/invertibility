...
(and (and (=> (bvsgt s (_ bv0 4)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 4)) (distinct t max))) (or (distinct t (_ bv0 4)) (distinct s (_ bv1 4))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 4))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 4))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
