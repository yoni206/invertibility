...
(and (=> (bvsge s (_ bv0 23)) (bvsgt s t)) (=> (bvslt s (_ bv0 23)) (bvsgt (bvlshr s (_ bv1 23)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 23))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 23))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
