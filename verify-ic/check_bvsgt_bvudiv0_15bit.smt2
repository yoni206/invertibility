...
(or (bvsgt (udivtotal (bvnot (_ bv0 15)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 15))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 15))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
