...
(and (=> (bvsge s (_ bv0 34)) (bvsge s t)) (=> (and (bvslt s (_ bv0 34)) (bvsge t (_ bv0 34))) (bvugt (bvsub s t) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 34))) (bvsge (uremtotal s x) t)))
  (=> (exists ((x (_ BitVec 34))) (bvsge (uremtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
