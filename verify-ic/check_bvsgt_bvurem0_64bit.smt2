...
(and (and (=> (bvsgt s (_ bv0 64)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 64)) (distinct t max))) (or (distinct t (_ bv0 64)) (distinct s (_ bv1 64))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 64))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 64))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
