...
(and (=> (bvsge s (_ bv0 41)) (bvsgt s t)) (=> (bvslt s (_ bv0 41)) (bvsgt (bvlshr s (_ bv1 41)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 41))) (bvsgt (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 41))) (bvsgt (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
