...
(or (bvsge (udivtotal (bvnot (_ bv0 13)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 13))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 13))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
