...
(and (=> (bvsge s (_ bv0 10)) (bvsge s t)) (=> (bvslt s (_ bv0 10)) (bvsge (bvlshr s (_ bv1 10)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 10))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 10))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
