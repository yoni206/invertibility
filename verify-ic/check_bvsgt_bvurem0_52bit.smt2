...
(and (and (=> (bvsgt s (_ bv0 52)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 52)) (distinct t max))) (or (distinct t (_ bv0 52)) (distinct s (_ bv1 52))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 52))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 52))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
