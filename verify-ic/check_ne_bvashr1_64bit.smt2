...
(and (or (not (= t (_ bv0 64))) (not (= s (_ bv0 64)))) (or (not (= t (bvnot (_ bv0 64)))) (not (= s (bvnot (_ bv0 64))))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 64))) (distinct (bvashr s x) t)))
  (=> (exists ((x (_ BitVec 64))) (distinct (bvashr s x) t)) (SC s t))
  )
 )
)
(check-sat)
