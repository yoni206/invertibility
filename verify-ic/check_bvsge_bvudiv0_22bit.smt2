...
(or (bvsge (udivtotal (bvnot (_ bv0 22)) s) t) (bvsge (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 22))) (bvsge (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 22))) (bvsge (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
