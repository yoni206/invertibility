...
(and (=> (bvsge s (_ bv0 16)) (bvsge s t)) (=> (bvslt s (_ bv0 16)) (bvsge (bvlshr s (_ bv1 16)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 16))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 16))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
