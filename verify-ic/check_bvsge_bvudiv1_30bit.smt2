...
(and (=> (bvsge s (_ bv0 30)) (bvsge s t)) (=> (bvslt s (_ bv0 30)) (bvsge (bvlshr s (_ bv1 30)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 30))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 30))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
