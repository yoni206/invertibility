...
(and (=> (bvsge s (_ bv0 28)) (bvsge s t)) (=> (bvslt s (_ bv0 28)) (bvsge (bvlshr s (_ bv1 28)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 28))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 28))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
