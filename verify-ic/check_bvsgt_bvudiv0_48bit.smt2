...
(or (bvsgt (udivtotal (bvnot (_ bv0 48)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 48))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 48))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
