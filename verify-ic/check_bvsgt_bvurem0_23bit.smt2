...
(and (and (=> (bvsgt s (_ bv0 23)) (bvslt t (bvnot (bvneg s)))) (=> (bvsle s (_ bv0 23)) (distinct t max))) (or (distinct t (_ bv0 23)) (distinct s (_ bv1 23))))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 23))) (bvsgt (uremtotal x s) t)))
  (=> (exists ((x (_ BitVec 23))) (bvsgt (uremtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
