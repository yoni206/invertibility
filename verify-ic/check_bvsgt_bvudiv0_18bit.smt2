...
(or (bvsgt (udivtotal (bvnot (_ bv0 18)) s) t) (bvsgt (udivtotal max s) t))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 18))) (bvsgt (udivtotal x s) t)))
  (=> (exists ((x (_ BitVec 18))) (bvsgt (udivtotal x s) t)) (SC s t))
  )
 )
)
(check-sat)
