...
(and (=> (bvsge s (_ bv0 19)) (bvsge s t)) (=> (bvslt s (_ bv0 19)) (bvsge (bvlshr s (_ bv1 19)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 19))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 19))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
