...
(and (=> (bvsge s (_ bv0 42)) (bvsge s t)) (=> (bvslt s (_ bv0 42)) (bvsge (bvlshr s (_ bv1 42)) t)))
)

(assert
 (not
  (and
  (=> (SC s t) (exists ((x (_ BitVec 42))) (bvsge (udivtotal s x) t)))
  (=> (exists ((x (_ BitVec 42))) (bvsge (udivtotal s x) t)) (SC s t))
  )
 )
)
(check-sat)
