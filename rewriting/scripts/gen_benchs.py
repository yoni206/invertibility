...
def main(dir_of_rw, output_dir, dir_of_templates):
    ...
    try:
      os.mkdir(output_dir)
        ...
        pass
    suffix_ltr = "(assert left)\n(assert (not right))\n(check-sat)\n"
    suffix_rtl = "(assert right)\n(assert (not left))\n(check-sat)\n"
    for t_f in templates_files:
        ...
        for rw_f in rewrite_files:
            ...
            output_content_ltr = template_content + "\n" + original_content + "\n" + suffix_ltr
            output_content_rtl = template_content + "\n" + original_content + "\n" + suffix_rtl
            with open(output_dir + "/" +  template_name + "/" +  "ltr_" + rw_f, 'w') as myfile:
                myfile.write(output_content_ltr)
            with open(output_dir + "/" +  template_name + "/" +  "rtl_" + rw_f, 'w') as myfile:
                myfile.write(output_content_rtl)
