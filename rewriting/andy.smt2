...
(define-fun left () Bool (< (intmul (+ x_w n) (intadd x_w x t) (int_sign_extend a_w a m)) (intmul (+ x_w n) x (int_sign_extend a_w a m))))
(define-fun right () Bool (and (not (= t 0)) (not (= a 0)) (= (< (intadd x_w x t) x) (intsgt a_w a 0))))

(assert right)
(assert (not left))
(check-sat)
(get-value (x_w t_w a_w n m x t a (int_sign_extend 1 1 0) (int_sign_extend 1 0 0) left right (intadd x_w x t)))
